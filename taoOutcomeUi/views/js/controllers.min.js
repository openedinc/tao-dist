define("taoOutcomeUi/controller/routes",[],function(){"use strict";return{Results:{actions:{viewResult:"controller/viewResult",index:"controller/inspectResults"}},ResultTable:{actions:{index:"controller/resultTable"}}}}),define("taoOutcomeUi/controller/inspectResults",["jquery","i18n","module","helpers","layout/actions/binder","uri","ui/feedback","util/encode","ui/datatable"],function($,__,module,helpers,binder,uri,feedback,encode){"use strict";function loadResults($container,model,classUri){var params={classUri:classUri};$(".inspect-results-grid",$container).empty().data("ui.datatable",null).datatable({
url:helpers._url("getResults","Results","taoOutcomeUi",params),model:model,actions:{view:function(id){var action={binding:"load",url:helpers._url("viewResult","Results","taoOutcomeUi")};binder.exec(action,{uri:uri.encode(id),classUri:classUri}||this._resourceContext)},delete:function(id){var confirmBox=$(".preview-modal-feedback"),cancel=confirmBox.find(".cancel"),save=confirmBox.find(".save");confirmBox.find(".modal-close");confirmBox.modal({width:500}),save.off("click").on("click",function(){$.ajax({url:helpers._url("delete","Results","taoOutcomeUi"),type:"POST",data:{uri:uri.encode(id)},dataType:"json",success:function(response){
response.deleted?(feedback().success(__("Result has been deleted")),loadResults($container,model,classUri)):feedback().error(__("Something went wrong ...")+"<br>"+encode.html(response.error),{encodeHtml:!1})}}),confirmBox.modal("close")}),cancel.off("click").on("click",function(){confirmBox.modal("close")})}}})}return{start:function(){var $container=$("#inspect-result");loadResults($container,$container.data("model"),$container.data("uri")),binder.register("endExpiredTests",function(){var url=this.url;$.ajax({type:"GET",url:url,dataType:"json",success:function(data){feedback().info(__("%s tests has been finished.",data.length.toString()))}})})}}}),
define("taoOutcomeUi/controller/resultTable",["jquery","lodash","i18n","helpers","module","ui/datatable","jquery.fileDownload"],function($,_,__,helpers,module){"use strict";return{start:function(){var conf=module.config(),$container=$(".result-table"),$filterField=$(".result-filter",$container),$tableContainer=$(".result-table-container",$container),filter=conf.filter||"lastSubmitted",classUri=conf.classUri||"",columns=[],groups={},buildGrid=function(url,action,done){$.ajax({url:url,dataType:"json",data:{filter:filter,classUri:classUri},type:"GET"}).done(function(response){response&&response.columns&&(columns="remove"===action?_.reject(columns,function(col){
return _.find(response.columns,function(resCol){return _.isEqual(col,resCol)})}):void 0!==response.first&&!0===response.first?response.columns.concat(columns):columns.concat(response.columns),_buildTable(done))})},_buildTable=function(done){var model=[];_.forEach(columns,function(col){model.push({id:col.prop||col.contextId+"_"+col.variableIdentifier,label:col.label,sortable:!1})}),$tableContainer.empty().data("ui.datatable",null).off("load.datatable").on("load.datatable",function(){$(".result-export",$container).off("click").removeClass("disabled").on("click",function(e){e.preventDefault(),
$.fileDownload(helpers._url("getCsvFile","ResultTable","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"POST",data:{filter:filter,columns:columns,classUri:classUri}})}),_.isFunction(done)&&(done(),done="")}).datatable({url:helpers._url("data","ResultTable","taoOutcomeUi",{filterData:filter}),querytype:"POST",params:{columns:columns,_search:!1,classUri:classUri},model:model})};$("[data-group]",$container).each(function(){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action");groups[group]=groups[group]||{},
groups[group][action]=$elt}),$container.on("click","[data-group]",function(e){e.preventDefault();var $elt=$(this),group=$elt.data("group"),action=$elt.data("action"),url=$elt.data("url");buildGrid(url,action,function(){_.forEach(groups[group],function($btn){$btn.toggleClass("hidden")})})}),buildGrid(helpers._url("getResultOfSubjectColumn","ResultTable","taoOutcomeUi",{filter:filter})),$filterField.select2({minimumResultsForSearch:-1}).select2("val",filter),$(".result-filter-btn",$container).click(function(e){filter=$filterField.select2("val"),_buildTable()})}}}),define("json!taoItems/preview/resources/device-list.json",function(){return{tablets:{
...
_computeScaleFactor(),_scale()}),itemUri=_itemUri,state=_state,overlay},show:show}}),define("taoOutcomeUi/controller/viewResult",["module","jquery","i18n","helpers","layout/section","taoItems/preview/preview","jquery.fileDownload"],function(module,$,__,helpers,section,preview){"use strict";return{start:function(){var conf=module.config(),$container=$("#view-result"),$filterField=$(".result-filter",$container);$filterField.select2({minimumResultsForSearch:-1}).select2("val",conf.filter||"all"),$(".result-filter-btn",$container).click(function(e){section.loadContentBlock(helpers._url("viewResult","Results","taoOutcomeUi"),{uri:conf.uri,classUri:conf.classUri,
filter:$filterField.select2("val")})}),$(".download",$container).on("click",function(e){var variableUri=$(this).val();$.fileDownload(helpers._url("getFile","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"POST",data:{variableUri:variableUri,deliveryUri:conf.classUri}})}),$(".preview",$container).on("click",function(e){e.preventDefault(),window.scrollTo(0,0),preview.init(helpers._url("forwardMe","ItemPreview","taoItems",{uri:$(this).data("uri")})),preview.show()})}}}),function(c){
