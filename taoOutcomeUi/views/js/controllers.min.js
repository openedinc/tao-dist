define("taoOutcomeUi/controller/routes",[],function(){"use strict";return{Results:{actions:{viewResult:"controller/viewResult",index:"controller/inspectResults"}},ResultTable:{actions:{index:"controller/resultTable"}}}}),define("taoOutcomeUi/plugins/results/action/view",["i18n","layout/actions/binder","core/plugin","util/url"],function(__,binder,pluginFactory,urlHelper){"use strict";return pluginFactory({name:"view",init:function(){var resultsList=this.getHost(),action={binding:"load",url:urlHelper.route("viewResult","Results","taoOutcomeUi")};resultsList.addAction({id:"view",label:__("View"),icon:"view",action:function(id){var context={id:id,
classUri:resultsList.getClassUri()};binder.exec(action,context)}})}})}),define("taoOutcomeUi/plugins/results/action/delete",["jquery","i18n","uri","core/plugin","util/url","util/encode","ui/dialog/confirm","ui/feedback"],function($,__,uri,pluginFactory,urlHelper,encode,dialogConfirm,feedback){"use strict";return pluginFactory({name:"delete",init:function(){var resultsList=this.getHost();resultsList.addAction({id:"delete",label:__("Delete"),icon:"delete",action:function(id){dialogConfirm(__("Please confirm deletion"),function(){$.ajax({url:urlHelper.route("delete","Results","taoOutcomeUi"),type:"POST",data:{uri:uri.encode(id)},dataType:"json"
}).done(function(response){response.deleted?(feedback().success(__("Result has been deleted")),resultsList.refresh()):(feedback().error(__("Something went wrong...")+"<br>"+encode.html(response.error),{encodeHtml:!1}),resultsList.trigger("error",response.error))}).fail(function(err){feedback().error(__("Something went wrong...")),resultsList.trigger("error",err)})})}})}})}),define("taoOutcomeUi/plugins/results/action/download",["jquery","i18n","core/plugin","util/url","jquery.fileDownload"],function($,__,pluginFactory,urlHelper){"use strict";return pluginFactory({name:"download",init:function(){var resultsList=this.getHost();resultsList.addAction({
id:"download",label:__("Download"),icon:"download",action:function(id){$.fileDownload(urlHelper.route("downloadXML","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"GET",data:{id:id,delivery:resultsList.getClassUri()}})}})}})}),define("taoOutcomeUi/component/results/pluginsLoader",["core/pluginLoader","taoOutcomeUi/plugins/results/action/view","taoOutcomeUi/plugins/results/action/delete","taoOutcomeUi/plugins/results/action/download"],function(pluginLoader,actionView,actionDelete,actionDownload){"use strict"
;return pluginLoader({action:[actionView,actionDelete,actionDownload]})}),define("taoOutcomeUi/component/results/areaBroker",["lodash","core/areaBroker"],function(_,areaBroker){"use strict";var requireAreas=["list"];return _.partial(areaBroker,requireAreas)}),define("tpl!taoOutcomeUi/component/results/list",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="component results-list">\n    <div class="list"></div>\n</div>'})}),
define("taoOutcomeUi/component/results/list",["jquery","lodash","uri","core/promise","ui/component","taoOutcomeUi/component/results/areaBroker","tpl!taoOutcomeUi/component/results/list","ui/datatable"],function($,_,uri,Promise,component,resultsAreaBroker,listTpl){"use strict";function resultsListFactory(config,pluginFactories){function pluginRun(method){var execStack=[];return _.forEach(plugins,function(plugin){_.isFunction(plugin[method])&&execStack.push(plugin[method]())}),Promise.all(execStack)}var resultsList,areaBroker,classUri,plugins={},actions=[];if(!_.isPlainObject(config))throw new TypeError("The configuration is required")
;if(!_.isString(config.classUri)||!config.classUri)throw new TypeError("The class URI is required");if(!_.isString(config.dataUrl)||!config.dataUrl)throw new TypeError("The service URL is required");if(!_.isPlainObject(config.model)&&!_.isArray(config.model))throw new TypeError("The data model is required");return classUri=uri.decode(config.classUri),resultsList=component({refresh:function(){return areaBroker.getListArea().datatable("refresh"),this},addAction:function(name,action){var descriptor;return _.isPlainObject(name)?descriptor=name:_.isPlainObject(action)?(descriptor=action,action.id=name):descriptor={id:name,label:name},
_.isFunction(action)&&(descriptor.action=action),descriptor.label||(descriptor.label=descriptor.id),actions.push(descriptor),this},getConfig:function(){return this.config},getClassUri:function(){return classUri},getActions:function(){return actions}}),resultsList.before("render",function(){var self=this;return areaBroker=resultsAreaBroker(this.$component,{list:$(".list",this.$component)}),_.forEach(pluginFactories,function(pluginFactory){var plugin=pluginFactory(self,areaBroker);plugins[plugin.getName()]=plugin}),pluginRun("init").then(function(){return pluginRun("render")}).then(function(){areaBroker.getListArea().empty().on("query.datatable",function(){
self.setState("loading",!0).trigger("loading")}).on("load.datatable",function(){self.setState("loading",!1).trigger("loaded")}).datatable({url:self.config.dataUrl,model:self.config.model,actions:actions})}).catch(function(err){return self.trigger("error",err),Promise.reject(err)})}).before("destroy",function(){var self=this;return pluginRun("destroy").then(function(){areaBroker.getListArea().empty()}).catch(function(err){self.trigger("error",err)})}).setTemplate(listTpl).init(config)}return resultsListFactory}),
define("taoOutcomeUi/controller/inspectResults",["jquery","lodash","module","core/logger","util/url","layout/actions/binder","layout/loading-bar","ui/feedback","taoOutcomeUi/component/results/pluginsLoader","taoOutcomeUi/component/results/list"],function($,_,module,loggerFactory,urlHelper,binder,loadingBar,feedback,resultsPluginsLoader,resultsListFactory){"use strict";function reportError(err){loadingBar.stop(),logger.error(err),err instanceof Error&&feedback().error(err.message)}var logger=loggerFactory("controller/inspectResults");return{start:function(){
var config=module.config()||{},$container=$("#inspect-result"),classUri=$container.data("uri"),listConfig={dataUrl:urlHelper.route("getResults","Results","taoOutcomeUi",{classUri:classUri}),model:config.dataModel,classUri:classUri};loadingBar.start(),_.forEach(config.plugins,function(plugin){plugin&&plugin.module&&(plugin.exclude?resultsPluginsLoader.remove(plugin.module):resultsPluginsLoader.add(plugin))}),$container.length&&resultsPluginsLoader.load().then(function(){resultsListFactory(listConfig,resultsPluginsLoader.getPlugins()).on("error",reportError).on("success",function(message){feedback().success(message)}).before("loading",function(){
loadingBar.start()}).after("loaded",function(){loadingBar.stop()}).render($(".inspect-results-grid",$container))}).catch(reportError),binder.register("download_csv",function(item){$.fileDownload(this.url,{httpMethod:"GET",data:{uri:item.uri}})})}}}),define("taoOutcomeUi/controller/resultTable",["jquery","lodash","i18n","helpers","module","ui/datatable","jquery.fileDownload"],function($,_,__,helpers,module){"use strict";return{start:function(){
var conf=module.config(),$container=$(".result-table"),$filterField=$(".result-filter",$container),$tableContainer=$(".result-table-container",$container),filter=conf.filter||"lastSubmitted",uri=conf.uri||"",columns=[],groups={},buildGrid=function(url,action,done){$.ajax({url:url,dataType:"json",data:{filter:filter,uri:uri},type:"GET"}).done(function(response){response&&response.columns&&(columns="remove"===action?_.reject(columns,function(col){return _.find(response.columns,function(resCol){return _.isEqual(col,resCol)})}):void 0!==response.first&&!0===response.first?response.columns.concat(columns):columns.concat(response.columns),_buildTable(done))})
},_buildTable=function(done){var model=[];_.forEach(columns,function(col){model.push({id:col.prop||col.contextId+"_"+col.variableIdentifier,label:col.label,sortable:!1})}),$tableContainer.empty().data("ui.datatable",null).off("load.datatable").on("load.datatable",function(){$(".result-export",$container).off("click").removeClass("disabled").on("click",function(e){e.preventDefault(),$.fileDownload(helpers._url("getCsvFile","ResultTable","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"POST",data:{filter:filter,
columns:JSON.stringify(columns),uri:uri}})}),_.isFunction(done)&&(done(),done="")}).datatable({url:helpers._url("data","ResultTable","taoOutcomeUi",{filterData:filter}),querytype:"POST",params:{columns:JSON.stringify(columns),_search:!1,uri:uri},model:model})};$("[data-group]",$container).each(function(){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action");groups[group]=groups[group]||{},groups[group][action]=$elt}),$container.on("click","[data-group]",function(e){e.preventDefault();var $elt=$(this),group=$elt.data("group"),action=$elt.data("action"),url=$elt.data("url");buildGrid(url,action,function(){_.forEach(groups[group],function($btn){
$btn.toggleClass("hidden")})})}),buildGrid(helpers._url("getResultOfSubjectColumn","ResultTable","taoOutcomeUi",{filter:filter})),$filterField.select2({minimumResultsForSearch:-1}).select2("val",filter),$(".result-filter-btn",$container).click(function(e){filter=$filterField.select2("val"),_buildTable()})}}}),define("json!taoItems/preview/resources/device-list.json",function(){return{tablets:{"193986c3715c81838870f908fa98d69a":{label:"Amazon Kindle Fire HDX 7″",width:960,height:600},"4e8eafab11aad1486992d7ee2c8c16ca":{label:"Apple iPad",width:1024,height:768},"5aac2932d9ad00d6d8a1604b8f9e4e8d":{label:"Google Nexus 10, Motorola Xoom, Xyboard",width:1280,
height:800},"9805d9753ad08c7630a9ee5418aa6c6c":{label:"Google Nexus 7",width:966,height:604},d4a431cedf4705d6bf2cba6d5788378b:{label:"Samsung Galaxy Tab 7.7, 8.9, 10.1",width:1280,height:800}},phones:{"1b89338531ef0155c8d6abc2ac02c81a":{label:"Apple iPhone 3GS",width:320,height:480,scaleFactor:1,dpWidth:320,dpHeight:480},"3f89da99d84214cde7df87ebe0699a6f":{label:"Apple iPhone 4",width:640,height:960,scaleFactor:2,dpWidth:320,dpHeight:1920},"0d36971a5b98b367f54ced708c9af849":{label:"Apple iPhone 5",width:640,height:1136,scaleFactor:2,dpWidth:320,dpHeight:2272},"026beffcc051af995660ebdede986ace":{label:"BlackBerry Z10",width:768,height:1280,scaleFactor:2,
dpWidth:384,dpHeight:2560},fd2dd53e667d6d49e4fa73356fa941dd:{label:"BlackBerry Z30",width:720,height:1280,scaleFactor:2,dpWidth:360,dpHeight:2560},"707584c1ae17d9b5b2cbe8603c91c147":{label:"Google Nexus 4",width:768,height:1280,scaleFactor:2,dpWidth:384,dpHeight:2560},"91823264de952e7f2347e07db5c4058b":{label:"Google Nexus 5",width:1080,height:1920,scaleFactor:3,dpWidth:360,dpHeight:5760},cd69817a6e147e8a4677389e56fcf568:{label:"Google Nexus S",width:480,height:800,scaleFactor:1.5,dpWidth:320,dpHeight:1200},"75994faa6a38e31e99b53fcd75534a33":{label:"HTC Evo, Touch HD, Desire HD, Desire",width:480,height:800,scaleFactor:1.5,dpWidth:320,dpHeight:1200},
...
_computeScaleFactor(),_scale()}),itemUri=_itemUri,state=_state,overlay},show:show}}),define("taoOutcomeUi/controller/viewResult",["module","jquery","i18n","util/url","layout/section","taoItems/preview/preview","jquery.fileDownload"],function(module,$,__,urlHelper,section,preview){"use strict";return{start:function(){var conf=module.config(),$container=$("#view-result"),$resultFilterField=$(".result-filter",$container),$classFilterField=$('[name="class-filter"]',$container),classFilter=JSON.parse(conf.filterTypes)||[];$resultFilterField.select2({minimumResultsForSearch:-1}).select2("val",conf.filterSubmission||"all")
;for(var i in classFilter)$('[value="'+classFilter[i]+'"]').prop("checked","checked");$(".result-filter-btn",$container).click(function(e){classFilter=[""],$classFilterField.each(function(){$(this).prop("checked")&&classFilter.push($(this).val())}),section.loadContentBlock(urlHelper.route("viewResult","Results","taoOutcomeUi"),{id:conf.id,classUri:conf.classUri,filterSubmission:$resultFilterField.select2("val"),filterTypes:classFilter})}),$(".download",$container).on("click",function(e){var variableUri=$(this).val();$.fileDownload(urlHelper.route("getFile","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),
failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"POST",data:{variableUri:variableUri,deliveryUri:conf.classUri}})}),$(".preview",$container).on("click",function(e){var $this=$(this),uri=$this.data("uri"),state=$this.data("state");e.preventDefault(),window.scrollTo(0,0),preview.init(urlHelper.route("forwardMe","ItemPreview","taoItems",{uri:uri}),state),preview.show()})}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
